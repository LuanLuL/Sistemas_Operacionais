...
void RamMemory::addProcess(MemoryPage process) {
    this->virtualAddresses.addAddress(process.id);
    this->listOfProcess.emplace(process.id, process);
}

MemoryPage RamMemory::getNextProcess() {
    if (this->listOfProcess.empty()) {
        throw runtime_error("RamMemory::getNextProcess(No processes in the queue)");
    }
    int processId = virtualAddresses.getNextAddress();
    MemoryPage nextProcess = this->listOfProcess[processId];
    listOfProcess.erase(processId);
    return nextProcess;
}

bool RamMemory::hasProcesses() {
    return !this->listOfProcess.empty();
}

int RamMemory::getNumberOfProcesses() {
    return this->listOfProcess.size();
}
